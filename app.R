...
warning = function(title, text) {
  showModal(modalDialog(title = title, text, easyClose = TRUE))
}
...
overbrug_me = function(spaargeld,
                       finproduct,
                       restbezit,
                       schuld,
                       type = "aanwas voor hvv") {
  schuld = schuld - 3400
  if (schuld < 0) {
    schuld = 0
  }
  ...
  aanwas = 0.0036 * spaargeld + 0.0617 * finproduct + 0.0617 * restbezit - 0.0257 *
    schuld
  if (type == "aanwas voor hvv") {
    return(aanwas)
  }
  if (type == "vermogen voor hvv") {
    return(grondslag)
  }
  ...
  vermogen_hvv = (spaargeld - (prp * spaargeld)) + (finproduct - (prp *
                                                                    finproduct)) + (restbezit - (prp * restbezit)) - (schuld - (prp * schuld))
  aanwas_hvv = 0.0036 * (spaargeld - (prp * spaargeld)) + 0.0617 * (finproduct - (prp *
                                                                                    finproduct)) + 0.0617 * (restbezit - (prp * restbezit)) - 0.0257 * (schuld - (prp *
                                                                                                                                                                    schuld))
  
  if (type == "aanwas na hvv") {
    return(aanwas_hvv)
  }
  if (type == "vermogen na hvv") {
    return(vermogen_hvv)
  }
...
gen_history = function(row, sd_rend, crisis = "nee") {
  aanwas_forfait = overbrug_me(row$spaargeld[1],
                               row$findproduct[1],
                               row$restbezit[1],
                               row$schuld[1])
...
verreken_verlies = function(data,
  ...
  for (y in c(1:length(c(start:end)))) {
      ...
      for (i in c(1:nrow(cf_data))) {
        ...
        if (cf_row$grondslag < drempel) {
          ...
          if (abs(data$grondslag[data$jaar == cf_row$jaar]) <= data$grondslag[which(data$jaar == year)] &
            ...
            if (data$grondslag[data$jaar == cf_row$jaar] < -rest) {
              data$grondslag[data$jaar == cf_row$jaar] = data$grondslag[data$jaar == cf_row$jaar] + rest
            } else {
              data$grondslag[data$jaar == cf_row$jaar] = 0
            }
    ...
    } else if (data$grondslag[data$jaar == year] < drempel &
               cb > 0) {
      cb_jaar = year - cb
      if (cb_jaar < 2026) {
        cb_jaar = 2026
...
calculate_variant_stats_macro = function(data = test,
                                         hvi = 1000,
                                         vv_drempel = 1000,
                                         cf = 9,
                                         cb = 1,
  ...
  for (i in 1:length(unique(data$id))) {
    ...
    temp$belasting = sapply(temp$grondslag, FUN = bepaal_belasting_totaal_adjusted)
    temp$belasting = sapply(temp_ideal$grondslag, FUN = bepaal_belasting_totaal_adjusted)
  ...
  gini_opbrengst = round(ineq(
    aggregate(belasting ~ jaar, data = data_new, FUN = sum)$belasting,
    type = "Gini"
  ), 2)
  
  # overbelasting
  vv_unlimited = sum(data_ideal$belasting, na.rm = T) * mpf
  overbelasting = ((budget_raming - vv_unlimited) / budget_raming) * 100
  
  return(
    list(
      budget_raming = budget_raming,
      gini_grondslag = gini_grondslag,
      gini_belasting = gini_belasting,
      gini_opbrengst = gini_opbrengst,
...
calculate_variant_stats_micro = function(data = test,
                                         hvi = 1000,
                                         vv_drempel = 1000,
                                         cf = 9,
                                         cb = 1,
  ...
  for (i in 1:length(unique(data$id))) {
    ...
    temp$belasting = sapply(temp$grondslag, FUN = bepaal_belasting_totaal_adjusted)
    temp_ideal$belasting = sapply(temp_ideal$grondslag, FUN = bepaal_belasting_totaal_adjusted)
...
gen_combi = function(dat_variant, dat_case, jaar = "alle jaren") {
  ...
  if (sum(dat_case$aanwas, na.rm = T) > 0) {
    belasting_perc = round(((
      belasting / sum(dat_case$aanwas, na.rm = T)
    ) * 100), 2)
  } else {
    belasting_perc = 0
  }
  # percentage verrekend verlies tov totaal verlies
  if (nrow(subset(dat_case, aanwas < 0) > 0)) {
    verlies = sum(subset(dat_case, aanwas < 0)$aanwas, na.rm = T)
  } else {
    verlies = 0
  }
  if (verlies > 0) {
    verrekend_verlies_perc = (sum(dat_case_verlies$cf, na.rm = T) + sum(dat_case_verlies$cb, na.rm = T)) /
      verlies
  } else {
    verrekend_verlies_perc = 0
  }
  # percentage grondslag tov aanwas
  if (sum(dat_case_verlies$grondslag, na.rm = T) > 0) {
    grondslag_perc = round((
      sum(dat_case_verlies$grondslag, na.rm = T) / sum(dat_case$aanwas, na.rm = T)
    ) * 100, 2)
  } else {
    grondslag_perc = 0
  }
  ...
  temp_case = data.frame(
    ...
    verlies = abs(round(sum(
      subset(dat_case_verlies, aanwas < 0)$aanwas, na.rm = T
    ), 2)),
    verrekend_verlies = round(
      sum(dat_case_verlies$cf, na.rm = T) +  sum(dat_case_verlies$cb, na.rm = T),
      2
    ),
    ...
    grondslag = round(sum(
      subset(dat_case_verlies, grondslag > 0)$grondslag, na.rm = T
    ), 2),
...
variant_stats = calculate_variant_stats_macro(
  hvi = 1000,
  vv_drempel = 1000,
  cf = 9,
  cb = 1,
    ...
    tabPanel(
      ...
      div(
        ...
        sidebarPanel(
          ...
          h5("Crisis"),
          helpText(
            "Heeft belastingplichtige een financiële crisis meegemaakt? Zo ja, dan simuleert de tool een vijfjarige crisis vanaf 2035."
          ),
          div(
        ...
        tabPanel(title = "Bewerk dataset", fluidPage(
          ...
          column(
            ...
            fluidRow(column(
              3,
              h5("download template"),
              downloadButton("download_template_variant_macro", label = "download template")
            ),
            column(
              9,
              h5("data opladen (.xlsx)"),
              fileInput(
                "upload_data_variant_macro",
                label = NULL,
                multiple = F,
                accept = ".xlsx",
                width = '100%',
          ...
          column(
            2,
            actionButton(
              inputId = "reset_data_variant_macro",
              label = "reset dataset",
              width = '100%'
            ),
            h4(),
            actionButton(
              inputId = "delete_variant_macro",
              label = "verwijder variant",
              width = '100%'
            ),
            h4(),
            downloadButton(
              "download_variants_macro",
              label = "opslaan",
              style = "width:100%;"
            )
          )
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          column(
            10,
            HTML(
              "De onderstaande tabel bevat <em>een schatting</em> van elk van de door u gespecificeerde variant voor de populatie.
                                   Bent u ontevreden met de dataset? In het onderstaande luik kunt u alle data te verwijderen middels de <em>reset</em> knop of een enkele <em>rij verwijderen</em>.
                                   ...
                                   <br><br>
                                   <ul>
                                   <li><b>budgettaire opbrengst</b>, m.n. de budgettaire opbrengst van de variant (in mld.);</li>
                                   <li><b>budget instabiliteit</b>, m.n. de mate waarin de opbrengst gelijkmatig verdeeld is over de jaren heen (0-1);</li>
                                   ...
                                   <em>Let op! De statistieken die hier worden gepresenteerd zijn <b>schattingen</b>, gebaseerd op een random steekproef (N = 10000, 3% van de populatie) van onze database.
                                   ...
                                   minder precies dan een raming. Wilt u een raming opvragen? U kunt voor één enkele variant een rapport genereren onder het tabblad genereer rapport </em>"
            ),
          ...
          column(
            ...
            h4(),
            actionButton(
              inputId = "delete__population_effects",
              label = "verwijder casus",
              width = '100%'
            ),
            h4(),
...
server = function(input, output) {
  ...
  observeEvent(input$add_case, {
    ...
    easteregg$vnw = "hij"
    easteregg$vnw[easteregg$geslacht == "vrouw"] = "zij"
    easteregg$vnw_bezit = "hem"
    easteregg$vnw_bezit[easteregg$geslacht == "vrouw"] = "haar"
    ...
    if (input$omschrijving %in% easteregg$omschrijving) {
      ...
      opmerkingen = c(
        paste0(
          "Vrouwen vergelijken met een auto? Dat zit er voor ",
          voornaam,
          " niet in."
        ),
        paste0(
          "We gaan op sjiek, mensen. Dit is ",
          voornaam,
          ". Ouder van twee volwassen dochters en
                en nee, ",
          vnw,
          " draagt niet het trouwpak van het eerste of tweede huwelijk, maar misschien
                wel van nummer drie. Ben jij die nummer drie? Bel ",
          vnw_bezit,
          " snel!"
        ),
        paste0(
          voornaam,
          " neemt nooit initiatief en doet nooit iets in het huishouden.
               Gun jezelf de tijd en met een beetje begeleiding heb je binnen 30 jaar een eigen ",
          voornaam,
          " in huis die wel de ",
          geslacht,
          " van je dromen is. Even geduld dus!"
        ),
        paste0(
          voornaam,
          " heeft helemaal geen behoefte aan een bakkie, ",
          vnw,
          " wil een moderne man
               die matcht en zelf zijn jasje dasje wast. Dus zie jij jezelf wel naast ",
          vnw_bezit,
          " in
               de spiegel staan, bel ",
          vnw_bezit,
          " dan snel."
        ),
        paste0(
          voornaam,
          " heeft het al niet zo op dominante vrouwen en ",
          vnw,
          " is er nu helemaal klaar mee.
               Ben jij dol op het huishouden en zoek jij een ",
          geslacht,
          " die zich het liefst laat
               verzorgen? Dan trakteert ",
          voornaam,
          " je graag op een bakkie -- of zelfs een achterbakkie
               (wink wink, nudge nudge) als je mazzel hebt."
        ),
        paste0(
          voornaam,
          " is heel erg gevoelig voor alcohol, maar kijkt erg nuchter naar de liefde. Wil jij
               een wijntje met ",
          vnw_bezit,
          " delen? Neem dan contact met ",
          vnw_bezit,
          " op!"
        ),
        paste0(
          voornaam,
          " vindt zichzelf te knap voor ",
          sample(easteregg$voornaam, size = 1),
          ".
               Ben jij wel knap genoeg? Bel dan snel!"
        ),
        paste0(
          "Croissantjes lekker voorverwarmd op een inductieplaat, eitjes in de airfryer pan hup in de oven,
               versgeperste sinaasappelsap met meer pitjes dan sap. Heerlijk wakker worden want ",
          voornaam,
        ...
        paste0(
          "Romantiek voor ",
          voornaam,
          " is lekker samenzitten met een goed boek onder een sterrenhemel.
               Maar niet in de Provence. Je vult weer vanalles voor ",
          vnw_bezit,
          " in. Daar heeft ",
          vnw,
          " veel moeite mee. Dus laat ",
          vnw_bezit,
          " in rust een boekje lezen."
        )
      ...
      popup(
        paste0(
          "Wat als ... ",
          input$omschrijving,
          " aan Lang Leve de Liefde (de meer genante versie van First Dates op SBS6) zou deelnemen?"
        ),
    ...
    if (input$omschrijving == "Stefan Smalbrugge") {
      popup_adjusted(
        "Ga jij Stefan ook zo missen?",
        "Met ingang van volgend jaar zal Stefan ons bij het Ministerie verlaten ten gunste van zijn warme nestje bij de Belastingdienst. Ga jij Stefan missen?",
        footer = tagList(
      ...
      observeEvent(input$stefan_ja, {
        popup_adjusted(
          ...
          footer = tagList(
            ...
            textInput(
              ...
              label = "Bericht",
              width = '100%',
              placeholder = "Schrijf hier een lief berichtje."
            ),
        ...
        observeEvent(input$stefan_ja_verstuur, {
          #Stefan Smalbrugge
          send.mail(
            from = "sjifra.de.leeuw@outlook.com",
            to = "sjifra.de.leeuw@gmail.com",
            subject = paste0(
              input$stefan_ja_naam,
              " van het Ministerie van Financiën mist je ... "
            ),
            body = paste0(
              input$stefan_ja_text,
              "\n\nMail hem/haar terug via ",
              input$stefan_ja_email
            ),
            smtp = list(
      ...
      observeEvent(input$stefan_nee, {
        popup_adjusted(
          ...
          footer = tagList(
            actionButton("stefan_nee_2", "Ik ben een monster"),
      ...
      observeEvent(input$stefan_ja_2, {
        ...
        observeEvent(input$stefan_ja_verstuur, {
          send.mail(
            ...
            to = "sjifra.de.leeuw@gmail.com",
            subject = paste0(
              input$stefan_ja_2_naam,
              " van het Ministerie van Financiën mist je ... "
            ),
            body = paste0(
              input$stefan_ja_2_text,
              "\n Mail hem/haar terug via ",
              input$stefan_ja_2_email
            ),
            smtp = list(
              host.name = "smtp.office365.com",
              port = 587,
    ...
    if (nrow(data) < 1) {
      ...
      if (verschil > 0) {
        text = paste0(
          text,
          "<b>Dat is ",
          number_to_money(verschil),
          " meer dan de grondslag die belastingplichtige zou hebben vóór toerekening van het heffingvrij vermogen onder de overbrugging wetgeving.</b> <br><br>"
        )
      }
      else if (verschil < 0) {
        text = paste0(
          text,
          "<b>Dat is ",
          number_to_money(abs(verschil)),
          " minder dan de grondslag die belastingplichtige vóór toerekening van het heffingvrij vermogen  zou hebben onder de overbrugging wetgeving.</b> <br><br>"
        )
      }
      else {
        text = paste0(
          text,
          "<b> Dat is exact evenveel als de grondslag die belastingplichtige vóór toerekening van het heffingvrij vermogen zou hebben onder de overbruggings wetgeving.</b> <br><br>"
        )
      }
  ...
  output$plot_aanwas_2026 = renderPlotly({
    ...
    df = data.frame(
      ...
      y = c(
        ...
        df$spaargeld * (df$finproduct_rendperc / 100),
        df$restbezit * (df$restbezit_rendperc / 100),
        df$schuld * (df$schuld_rendperc / 100),
        (
          df$spaargeld * (df$spaargeld_rendperc / 100) + df$spaargeld * (df$finproduct_rendperc /
                                                                           100) + df$restbezit * (df$restbezit_rendperc / 100) - df$schuld * (df$schuld_rendperc /
                                                                                                                                                100)
        )
      ),
  ...
  observeEvent(input$reset_variant_case_effects, {
    ...
    popup_adjusted(title = "Waarschuwing", text = "U staat op het punt alle casus en variant data te verwijderen.
        Weet u zeker dat u deze actie wil voltooien?", footer = tagList(
  ...
  gen_tab_micro = function() {
    ...
    if (input$micro_3_select_year == "Alle jaren") {
      ...
      case_dat = subset(case_dat, jaar == as.numeric(input$micro_3_select_year))
    }
    
    # aanwas
    if (nrow(subset(case_dat, aanwas > 0)) > 0) {
      aanwas = sum(subset(case_dat, aanwas > 0)$aanwas , na.rm = T)
    } else {
      aanwas = 0
    }
    ...
    if (variant_dat$hvi > 0) {
      hvi_tekst = aanwas - aanwas_na_hvi
    } else {
      hvi_tekst = "n.v.t."
    }
    if (variant_dat$cf > 0 |
    ...
    tab = rbind(
        ...
        c("Belasting ...", empty),
        c(paste0("... waarvan in schijf 1 (", "n.v.t.", ")") , empty),
        c(paste0("... waarvan in schijf 2 (", "n.v.t.", ")"), empty),
        c(paste0("... waarvan in schijf 3 (", "n.v.t.", ")"), empty)
      ) %>% data.frame() %>%
        setNames(c("", "Bedrag in €"))
  ...
  gen_micro_text = function() {
    ...
    }} else {text = "Geen data beschikbaar. Voeg data toe in Stap 1 en Stap 2."}
      ...
      out[[i]] = data.frame(
        t(select(
          cbind(
            ...
            data.frame(
              calculate_variant_stats_micro(
                data = case_data(),
                hvi = data$hvi[i],
                vv_drempel = data$verlies_drempel[i],
                cf = data$cf[i],
                cb = data$cb[i],
                s2 = data$schijf_2[i],
                s3 = data$schijf_3[i],
  ...
  output$plot_micro_variant_1 = renderPlotly({
    ...
    for (i in c(1:length(unique(case_dat$id)))) {
      ...
      temp[[i]] = data.frame(
        ...
        type = factor(
          c("aanwas", "grondslag", "belasting"),
          levels = c("aanwas", "grondslag", "belasting"),
          ordered = T
        ),
        text = c(paste0(
  ...
  observeEvent(input$add_variant_macro, {
    # 1. als schijf 2 lager ligt dan hvi
    if (!is.na(input$schijf_2_macro) &
        input$schijf_2_macro <= input$hvi_macro) {
      warning(
        "Casus niet toegevoegd",
        "De ondergrens van de tweede schijf kan niet lager zijn dan of gelijk zijn aan het heffing vrij inkomen."
      )
      ...
      # 2. als schijf 3 lager ligt dan schijf 2 of hvi
    } else if (!is.na(input$schijf_3_macro) &
               input$schijf_3_macro <= input$hvi_macro |
               !is.na(input$schijf_3_macro) &
               !is.na(input$schijf_2_macro) &
               input$schijf_3_macro <= input$schijf_2_macro) {
      warning(
        "Casus niet toegevoegd",
        "De ondergrens van de derde schijf kan niet lager zijn dan of gelijk zijn aan de ondergrens van schijf 2 of het heffingvrij inkomen."
      )
      ...
      if (is.null(input$upload_data_variant_macro)) {
        data = variant_data_input_macro()
      }
      if (!is.null(input$upload_data_variant_macro)) {
        data = upload_data_variant_macro$data
      }
      
      if (isolate(input$naam_variant_macro) %in% data$variant) {
        naam_variant_new = paste0(isolate(input$naam_variant_macro), " ", nrow(data) +
                                    1)
      } else {
      ...
      # variant toevoegen
      if (is.null(input$upload_data_variant_macro)) {
        variant_data_input_macro() %>%  bind_rows(dat) %>% variant_data_input_macro()
      } else {
        upload_data_variant_macro$data = upload_data_variant_macro$data %>%  bind_rows(dat)
      }
  ...
  observeEvent(input$upload_data_variant_macro, {
    upload_data_variant_macro$data = gen_upload_data_variant_macro()
  })
  ...
  observeEvent(input$reset_data_variant_macro, {
    if (is.null(input$upload_data_variant_macro)) {
      variant_data_input_macro() %>% filter(row_number() %in% -1) %>% variant_data_input_macro()
    } else {
      upload_data_variant$data = gen_empty_data_variant()
    }
  ...
  output$download_variants_macro = downloadHandler(
    filename = function() {
      paste("sandbox3_variants_macro.xlsx", sep = "")
    },
    content = function(file) {
      write_xlsx(selected_data_variant_macro(input$upload_data_variant_macro),
                 file)
    }
  )
  ...
  output$variant_tekst_macro = renderText({
    if (is.null(input$upload_data_variant_macro)) {
      dat_variant = variant_data_input_macro()
    } else {
      dat_variant = upload_data_variant_macro$data
    }
